...
## Supplemental Codex Churn Case
- #65 (Public view share cleanup and remove Wrapped 2025): Codex reviews repeated with low-signal feedback; churn driven by internal edge-case fixes.
...
## PR #65: Public view share cleanup and remove Wrapped 2025 (Codex churn focus)

### Problem Theme
Codex reviews were repeatedly re-triggered, but feedback was generic; churn came from internal fixes around public share auth boundaries.

### Evidence
- Codex reviews on commits `0bb3f51624`, `2538f4dac1`, `5e7536908f` were generic (no actionable feedback).
- Fix commits in this PR: `fix: accept non-jwt bearer before public fallback`, `fix: detect invalid public links from string errors`, `fix: require share token for public mode`, `fix: avoid expiring auth on share token 401`.

### 5 Whys
1. Why did Codex review repeat? Code kept changing for edge-case auth handling before each new review.
2. Why were edge cases missed earlier? Public share auth rules were not fully specified.
3. Why not specified? Design focused on cleanup, not explicit public access policy.
4. Why wasn’t policy explicit? No checklist to require share-token/401 behavior documentation.
5. Why no checklist? Public exposure gates weren’t enforced for this change.

### Fishbone (Root Cause Categories)
- People: Assumed cleanup was low-risk.
- Process: No public exposure checklist enforced for Codex review.
- Tools: No dedicated edge-case test harness for share links.
- Requirements: Public access policy not enumerated.
- Code: Auth boundary logic evolved through patches.
- Tests: Missing regression for non-JWT bearer and share-token 401.
- Communication: Codex review context did not explain policy deltas.

### Stage Attribution
- Primary: Implementation
- Secondary: Design, Testing, Review Packaging
...
- Command: `node -e "const fs=require('fs'); const csv=fs.readFileSync('docs/retrospective/2026-01-13-pr-retro.csv','utf8'); if(!csv.includes('review_signals')) process.exit(1); console.log('csv-ok');"`
- Result: PASS (csv-ok)

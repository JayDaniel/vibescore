...
### Evidence
- Follow-up PRs #58-#63: preserve provider prefixes, hourly model filter selection, non-UTC heatmap alias filtering, prefixed model identity.
- PR comments reference alias timeline bucketing and missing model columns in raw/hourly paths.
...
### Evidence
- Fix PRs and docs: link-code exchange RPC path mismatch and records API fallback; idempotency and request_id persistence; UI expiry refresh.
- PR gate notes highlight uncovered live E2E exchange.
...
### Evidence
- Codex review flagged that model breakdown sorting used total_tokens instead of billable_total_tokens.
- Follow-up PR #57: prefer stored billable totals in UTC hourly aggregates.
...
### Evidence
- Follow-up fixes: trend hover width alignment, trend single-point color, header button style restore, heatmap null mock guard.
- PR gate notes: no manual live-data verification or cross-browser visual regression.
...
### Evidence
- Post-merge fixes: read public profile fields (nickname/profile/metadata) instead of raw_user_meta_data; force pixel avatar in public view.

/**
 * Supabase 数据源 API 实现
 * 基于 vibescore-old/dashboard 的实现适配
 * 仅在 VITE_DATA_SOURCE=supabase 时使用
 */
...
const BACKEND_RUNTIME_UNAVAILABLE =
  "Backend runtime unavailable (Supabase). Please retry later.";

// Supabase Edge Functions 使用 vibescore-* 路径命名
const PATHS = {
...
function resolveBaseUrl(baseUrl) {
  ...
  const candidate = raw || String(getSupabaseUrl() || "").trim();
  if (!candidate) throw new Error("Missing Supabase baseUrl");
...
function buildAuthHeaders(accessToken) {
  ...
  if (hasAccessTokenValue(accessToken)) {
...
async function requestWithFallbackPost({
  ...
  try {
    return await requestPostJsonOnce({
      baseUrl,
      path: primaryPath,
      accessToken,
      body,
...
async function requestGetJson({ baseUrl, path, accessToken, params, fetchOptions }) {
...
async function requestPostJsonOnce({ baseUrl, path, accessToken, body, fetchOptions }) {
  const url = new URL(path, baseUrl);
  const headers = {
...
async function parseJsonOrThrow(res) {
  ...
  const message = extractBackendErrorMessage(text) || `HTTP ${res.status}`;
...
function extractBackendErrorMessage(bodyText) {
